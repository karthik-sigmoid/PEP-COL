...
def scen_parse_contents(contents, filename, date):
    ...
    try:
        if 'csv' in filename:
            ...
            sales_pred = predict_sales(df, df_sales_pred_scenario, df_coef, model_features_data)
            # print(sales_pred.head(3))
    ...
    return html.Div([
        ...
        html.Div([
            dbc.Row([
                ...
                dtc.Carousel([
                        dcc.Graph(id='simulate-div1', style = {'border-right': '1px solid grey', 'border-left': '1px solid grey', 
                                                            'margin-top': '5px', 'margin-bottom': '10px'}),
                        dcc.Graph(id='simulate-div2', style = {'border-right': '1px solid grey', 'border-left': '1px solid grey',
                                                            'margin-top': '5px', 'margin-bottom': '10px'}),
                        dcc.Graph(id='simulate-div3', style = {'border-right': '1px solid grey', 'border-left': '1px solid grey',
                                                            'margin-top': '5px', 'margin-bottom': '10px'})
                    ],
                                slides_to_scroll=1,
                                swipe_to_slide=True,
...
def download_as_excel(n_clicks):
    ...
    if not n_clicks:
        ...
        for key in df_sheets.keys():
            # df_sheets[key].drop(0,axis=0,inplace=True)
...
def get_table(data, cnl, mfg):
    ...
    fixed_cols = ['Manufacturer','Channel','Date','Month',
                  'Baseline Units Prediction','User modified Units Prediction',
                ...
                dbc.Row([
                    dbc.Col([
                        dcc.Dropdown(id='xaxis-data', placeholder = placeholder,
                                ...
                                style = {"display":'inline-block', 'color': 'black', 'font-family': 'Verdana', 'font-size': '14px',
                                            'width':300,
                            })
                    ], width=4.5),
                    dbc.Col([
...
def make_sales_graphs(data, cnl, mfg):
    ...
    fig_units = go.Figure()
    fig_units.add_trace(go.Scatter(x= df['Date'], y = df['Baseline Units Prediction'],
                                   name= 'Baseline Units Prediction',
                                   line=dict(color='#015CB4')))
    fig_units.add_trace(go.Scatter(x= df['Date'], y = df['User modified Units Prediction'],
                                 name= 'User modified Units Prediction',
                                 marker= dict(color= '#C9002B')))
    fig_units.update_layout(title= "Units Simulation - Baseline Vs User Modified",
                            yaxis_title = 'Units (10^3)', paper_bgcolor= '#F9F9F9', plot_bgcolor= '#F9F9F9',
                            legend= dict(orientation= 'h', title=None))

    return fig_sales, fig_units, baseline_som, modified_som, baseline_sales, modified_sales
